...
// Required data: phone
// Optional data: non
// @TODO Only let an authenticated user access their object. Don't let them access anyones.
handlers._users.get = function (data, callback){
    // Check that the phone number is valid
    var phone = typeof(data.queryStringObject.phone) == 'string' && data.queryStringObject.phone.length === 12 ? data.queryStringObject.phone : false
    if (phone){
        _data.read('users', phone, function (err, data){
            if (!err && data){
...
// @TODO only let an authenticated user update their own object. Don't let the update someone else's
...
// @TODO only let an authenticated used delete their object. Dont let the delete anyone else's
